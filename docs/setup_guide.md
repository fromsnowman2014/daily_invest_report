...
3.  **Stock List 헤더 작성:**
    *   `Stock List` 시트의 첫 번째 행(A1:F1)에 아래 내용을 입력합니다.
    *   **A1**: `Ticker`
...
4.  **테스트 데이터 입력 (권장):**
    ...
    *   예시:
        *   `NVDA` | `2024-05-20` | `2024-06-01` | `100.5` | `AI 대장주` | `반도체`
        *   `AAPL` | `2024-01-01` | | | `애플 인텔리전스 기대` | `Tech`
...
### 3.1. Financial Modeling Prep (FMP) API Key 준비
*   **발급처:** [FMP Developer Portal](https://site.financialmodelingprep.com/developer/docs)
*   **Key:** Dashboard에서 복사한 Key 준비 (예: `abc12345...`)

### 3.2. Google Gemini API Key 준비
...
4.  아래 두 가지 속성을 추가하고 저장합니다.
    *   **속성(Property):** `FMP_API_KEY`
        *   **값(Value):** (복사해 둔 FMP Key 붙여넣기)
...
*   [ ] Google Sheet에 `Stock List` 시트가 있고 헤더가 작성됨.
*   [ ] `Stock List`에 테스트용 종목(NVDA 등)이 1개 이상 있음.
*   [ ] Apps Script 프로젝트가 생성됨.
*   [ ] Apps Script 설정 > 스크립트 속성에 `FMP_API_KEY`와 `GEMINI_API_KEY`가 저장됨.
